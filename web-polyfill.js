...
if (isWeb) {
  ...
  const WebAsyncStorage = {
    ...
    setItem: async (key, value) => {
      try { localStorage.setItem(key, value); } catch (e) { throw e; }
    },
    removeItem: async (key) => {
      try { localStorage.removeItem(key); } catch (e) { throw e; }
    },
    getAllKeys: async () => {
      try { return Object.keys(localStorage); } catch { return []; }
    },
    multiGet: async (keys) => {
      try { return keys.map(k => [k, localStorage.getItem(k) ?? null]); } catch { return keys.map(k => [k, null]); }
    },
    multiSet: async (keyValuePairs) => {
      try { keyValuePairs.forEach(([k, v]) => localStorage.setItem(k, v)); } catch (e) { throw e; }
    },
    multiRemove: async (keys) => {
      try { keys.forEach(k => localStorage.removeItem(k)); } catch (e) { throw e; }
    },
    clear: async () => {
      try { localStorage.clear(); } catch (e) { throw e; }
    },
  };
