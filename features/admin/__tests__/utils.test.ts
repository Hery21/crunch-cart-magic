...
function makeTx(overrides: Partial<Transaction> & { id: string; timestamp: string }): Transaction {
  return {
    id: overrides.id,
...
describe('filterTransactions', () => {
  const txToday = makeTx({ id: 'INV-001', timestamp: TODAY_ISO, grandTotal: 10000 });
  const txYesterday = makeTx({ id: 'INV-002', timestamp: YESTERDAY_ISO, grandTotal: 20000 });
  const txLastWeek = makeTx({ id: 'INV-003', timestamp: LAST_WEEK_ISO, grandTotal: 30000 });
...
describe('getTodayStats', () => {
  const txToday1 = makeTx({ id: 'INV-001', timestamp: TODAY_ISO, grandTotal: 10000 });
  const txToday2 = makeTx({ id: 'INV-002', timestamp: TODAY_ISO, grandTotal: 20000 });
  const txYesterday = makeTx({ id: 'INV-003', timestamp: YESTERDAY_ISO, grandTotal: 5000 });
