...
import { DEFAULT_SETTINGS, DEFAULT_PRICES, type Settings, type Transaction, type CartItem } from './pos-types';
  ...
  try {
    ...
    for (const k of Object.keys(DEFAULT_PRICES) as (keyof typeof DEFAULT_PRICES)[]) {
      ...
      if (v && typeof v === 'object' && 'regular' in v && 'jumbo' in v) {
        ...
        migrated[k] = {
          regular: { normal: (v as Record<string,number>).normal ?? DEFAULT_PRICES[k].regular.normal, kuantar: (v as Record<string,number>).kuantar ?? DEFAULT_PRICES[k].regular.kuantar },
...
export async function loadCart(): Promise<CartItem[]> {
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.map((i: Record<string, unknown>) => ({ size: 'regular', ...i })) as CartItem[];
    }
    return [];
  } catch {
    return [];
